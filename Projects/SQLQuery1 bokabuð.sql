...
kennitalavið  nvarchar (50) foreign key references Vidskiptavinir (kt),
);
...
Select Titill, Greiðslumati
from Pantanir
join Baekur on Pantanir.ISbnNR = Baekur.isbn
where Baekur.Titill = 'Ljónið á hæðinni'

Select Greiðslumati, COUNT(Greiðslumati) as 'fjöldi'
from Pantanir
group by Greiðslumati

Select AVG(verð) as 'meðalverð', count(Titill) as 'fjöldi', sum(verð) as 'heildarverð'
from Baekur

select kennitalavið, COUNT(kennitalavið) as 'fjöldi pantana á viðskiptavin'
from Pantanir
group by kennitalavið